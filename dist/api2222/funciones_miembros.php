...
function registrarMiembro($miembro){
    ...
    nombreContacto, telefonoContacto, imagen, fechaRegistro, cedula) 
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    $miembro->telefonoContacto, $imagen, date("Y-m-d H:i:s"), $miembro->cedula
];
...
function actualizarMembresia($matricula, $idMembresia, $duracion, $fecha){
    ...
    $parametros = [$idMembresia, 'ACTIVO', $fecha, $duracion, $matricula];
...
function editarMiembro($miembro) {
    ...
    sufreEnfermedad = ?, tieneSeguro = ?, enfermedad = ?, institucion = ?, 
    nombreContacto = ?, telefonoContacto = ?, imagen = ?, cedula = ? WHERE id = ?";
    $parametros = [$miembro->nombre, $miembro->telefono, $miembro->direccion, 
    $miembro->edad, $miembro->sufreEnfermedad, $miembro->tieneSeguro, $miembro->enfermedad, 
    $miembro->institucion, $miembro->nombreContacto,
    $miembro->telefonoContacto, $miembro->imagen, $miembro->cedula, $miembro->id];
    return editar($sentencia, $parametros);
}
